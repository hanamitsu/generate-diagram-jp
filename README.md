...
**3つのスタイルモード**を搭載。学術論文風の `academic`、グラレコ風の `graphic-note`、そして両者の良さをバランスよく取り入れた **`balanced`（デフォルト）** から選択できる。特に指定しなければ balanced モードで生成される。
...
テキスト記事に図解を差し込んだ実例（クリックで記事全文へ）:

- [家康流「サンドイッチ叱り」](docs/07_ieyasu_sandwich_shikari.md) — マネジメント手法を3ステップフロー＋心理効果の2枚で図解
- [Anthropic vs ペンタゴン](docs/08_anthropic_pentagon.md) — AI安全と国家安全保障の対立構造＋世論反応を2枚で図解
